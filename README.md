...
- **数字卡片**：键盘输入即在画布生成数字，随手拖动整理；单击立刻激活编辑，输入直接覆盖旧值
...
- v0.3.3 C 键清空画布（可撤销）；单击数字立刻激活编辑、输入覆盖旧值
- v0.4.0 线框/引线 1px 贴身并随缩放一致；一元函数引线修复；放置方向优先级
  （右下45°→右上→左上→左下→正上）；类 tan 的 S 形引线
